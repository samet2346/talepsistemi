...
export default function AcikTalepler() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        ...
        setRequests(results.map(mapJobToRequest));
      } catch (err) {
  ...
  const filteredRequests = requests.filter(req => {
    const matchCategory = categoryFilter === "" || req.category.includes(categoryFilter);
    const matchLocation = locationFilter === "" || req.location === locationFilter;
          ...
          <div className="space-y-5">
            {filteredRequests.length === 0 ? (
              ...
              filteredRequests.map(req => (
                <RequestCard key={req.id} request={req} role="usta" /> 
              ))
            )}
          </div>
