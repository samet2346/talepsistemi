...
export default function DegerlendirmeSayfasi() {
  ...
  // API dökümanına (ReviewRequest) uygun state isimlendirmesi
  ...
  // UI Status State (Alert yerine modern bildirimler için)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. İşi Tamamla (İlanı kapatır)
      await jobService.completeJob(id);
      
      // 2. Detaylı Yorum Yap (API şemasına %100 uygun veri gönderimi)
      await jobService.submitReview({ 
        job: parseInt(id, 10), // API job ID'sini integer bekliyor
        rating_quality: ratings.rating_quality,
        rating_speed: ratings.rating_speed,
        rating_price_loyalty: ratings.rating_price_loyalty,
      ...
      // GELİŞTİRME AŞAMASI İÇİN YEDEK (API çalışmazsa testi geçmek için açabilirsin)
      /*
      setStatus('success');
      setTimeout(() => {
        router.push('/taleplerim');
  ...
  // Modernize edilmiş, mobilde kırılmayan yıldız komponenti
